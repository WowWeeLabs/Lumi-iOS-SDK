...
- (void)setJoystickCenter:(UIImageView *)joystickCenter {
    ...
    [self.joystickCenter setFrame:CGRectMake(0, 0, self.frame.size.width/3, self.frame.size.height/3)];
...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event {
    for(UITouch *touch in touches){
        if(self.touchToTrack == touch){
            ...
            float maxX = sinf(angle) * (self.frame.size.width - self.joystickCenter.frame.size.width) / 2;
            float maxY = cosf(angle) * (self.frame.size.height - self.joystickCenter.frame.size.height) / 2;
...
- (CGVector)joystickVector {
    CGVector vector = CGVectorMake((self.joystickCenter.center.x - self.joystickCenter.frame.size.width/2 -  (self.frame.size.width - self.joystickCenter.frame.size.width)  / 2) / ((self.frame.size.width - self.joystickCenter.frame.size.width)/ 2),
                                   -(self.joystickCenter.center.y - self.joystickCenter.frame.size.height/2 - (self.frame.size.height - self.joystickCenter.frame.size.height) / 2) / ((self.frame.size.height - self.joystickCenter.frame.size.height)/ 2));
